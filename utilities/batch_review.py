...
def organize_leftover_htmls():
    ...
    try:
        # Get project root (one level up from utilities/)
        script_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        html_dir = os.path.join(script_dir, "test_outputs", "html")
        ...
        if html_files:
            ...
            if moved_count > 0:
                print(f"📁 Organized {moved_count} HTML file(s) into test_outputs/html/ folder")
    except:
...
def run_review(workarea_path, output_file, ipo=None, skip_validation=False, section=None):
    ...
    try:
        ...
        launcher_path = os.path.join(project_root, "avice_wa_review_launcher.csh")
...
def main():
    ...
    # Get project root (one level up from utilities/) to locate AGUR_UNITS_TABLE.csv
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    default_units_table = os.path.join(project_root, "agur_release_tracking", "AGUR_UNITS_TABLE.csv")
    ...
    parser.add_argument("--output-dir", default="test_outputs/batch_review_results",
                       help="Output directory for results (default: test_outputs/batch_review_results)")
