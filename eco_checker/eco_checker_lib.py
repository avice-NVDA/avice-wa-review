...
class ECOCheckerLib:
    ...
    def is_sequential_cell(self, cell_type: str) -> bool:
        """
        Check if cell type is a sequential cell (flop/latch) vs clock distribution cell
        
        Sequential cells appear in ClockTree.rpt because their CLOCK PINS receive clocks,
        but they are DATA PATH elements. It's allowed to change them to any flop/latch type.
        
        Only CLOCK DISTRIBUTION cells (buffers, inverters, clock gates) should be restricted.
        
        Args:
            cell_type: Cell type string (e.g., "SDFQSC2SE1SRLPD2T6HVT")
        
        Returns:
            True if cell is a sequential element (flop/latch), False otherwise
        """
        if not cell_type:
            return False
        ...
        # Sequential cell patterns (flops and latches)
        sequential_patterns = [
            'SDF',   # Scan flops
            'EDF',   # Enhanced scan flops  
            'DFC',   # Flops with clear
            'DFN',   # Negative edge flops
            'DFQ',   # Standard flops
            'DF',    # General flops (must be after more specific patterns)
            'LNQ',   # Data latches
            'LND',   # Data latches
        ]
        
        for pattern in sequential_patterns:
            if pattern in cell_upper:
                return True
    ...
    def analyze_vt_statistics(self, eco_commands: List[str]) -> Dict[str, int]:
        """
        Analyze voltage threshold (VT) statistics from ECO commands
        
        Counts how many cells are changed to LVT, HVT, SLVT, ULVT, etc.
        
        Args:
            eco_commands: List of ECO command lines
        
        Returns:
            Dict with VT type counts: {'LVT': count, 'HVT': count, ...}
        """
        vt_stats = defaultdict(int)
        
        for command_line in eco_commands:
            cell_type = None
            
            # Extract cell type from various command patterns
            # Pattern 1: ecoChangeCell -inst {INSTANCE} -cell {CELLTYPE}
            match = re.search(r'ecoChangeCell\s+-inst\s+\{[^}]+\}\s+-cell\s+\{([^}]+)\}', command_line)
            if match:
                cell_type = match.group(1)
            
            # Pattern 2: ecoAddRepeater ... -cell {CELLTYPE}
            if not cell_type and 'ecoAddRepeater' in command_line:
                match = re.search(r'-cell\s+\{([^}]+)\}', command_line)
                if match:
                    cell_type = match.group(1)
            
            # Pattern 3: size_cell INSTANCE CELLTYPE
            if not cell_type and command_line.startswith('size_cell'):
                match = re.match(r'size_cell\s+\S+\s+(\S+)', command_line)
                if match:
                    cell_type = match.group(1)
            
            # Pattern 4: insert_buffer NET CELLTYPE INSTANCE
            if not cell_type and command_line.startswith('insert_buffer'):
                match = re.match(r'insert_buffer\s+\S+\s+(\S+)', command_line)
                if match:
                    cell_type = match.group(1)
            
            # Check VT type in cell name
            if cell_type:
                cell_upper = cell_type.upper()
                # Check for various VT types (order matters - check longer patterns first)
                if 'SLVT' in cell_upper:
                    vt_stats['SLVT'] += 1
                elif 'ULVT' in cell_upper:
                    vt_stats['ULVT'] += 1
                elif 'LVT' in cell_upper:
                    vt_stats['LVT'] += 1
                elif 'HVT' in cell_upper:
                    vt_stats['HVT'] += 1
                elif 'RVT' in cell_upper:
                    vt_stats['RVT'] += 1
        
        return dict(vt_stats)
    ...
    def check_inst_not_allowed_on_clocks(self, eco_commands: List[str],
        ...
        CRITICAL DISTINCTION:
          - Sequential cells (flops/latches): Data path elements with clock PINS in tree -> ALLOWED to change
          - Clock distribution cells (buffers/inverters/gates): Drive clocks -> RESTRICTED by allowed patterns
        ...
        for command_line in eco_commands:
            ...
            if instance_name and cell_type:
                # CRITICAL: Skip check for sequential cells (flops/latches)
                # Sequential cells appear in ClockTree.rpt because their CLOCK PINS (e.g., CP)
                # receive clock signals, but the cells themselves are DATA PATH elements.
                # Only CLOCK DISTRIBUTION cells should be restricted.
                if self.is_sequential_cell(cell_type):
                    skipped_sequential_count += 1
                    continue
        ...
        # Print summary of skipped sequential cells (verbose mode only)
        if self.verbose and skipped_sequential_count > 0:
            print(f"  Skipped {skipped_sequential_count} sequential cell(s) (flops/latches are data path elements)")
