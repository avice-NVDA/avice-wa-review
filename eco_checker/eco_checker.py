...
def format_violations_summary(violations: List[Dict], check_type: str) -> str:
    """
    Format violations summary with 1 example per cell type (non-verbose mode)
    ...
    # Show first 2 cell types with 1 example each
    for idx, (cell_type, viols) in enumerate(sorted(by_type.items(), key=lambda x: len(x[1]), reverse=True)):
        if idx >= 2:  # Limit to 2 cell types
            remaining_types = len(by_type) - 2
            output.append(f"  ... and {remaining_types} more cell type(s) (use -v for details)")
            break
        
        output.append(f"  Cell Type '{cell_type}': {len(viols)} instance(s)")
        # Show only 1 example
        v = viols[0]
        output.append(f"    Example: {v['instance']}")
        output.append(f"    Command: {v['command'][:80]}...")
        if len(viols) > 1:
            output.append(f"    ... and {len(viols) - 1} more instance(s) of this type")
...
def format_violations_by_type(violations: List[Dict], check_type: str) -> str:
    """
    Format violations grouped by cell type (verbose mode - shows up to 3 examples per type)
...
Author: Alon Vice (avice@nvidia.com)
    ...
    for idx, eco_file in enumerate(args.eco_files, 1):
        ...
        if not args.quiet:
            # Show workarea (always)
            if refs['workarea']:
                print(f"  Workarea:     {refs['workarea']}")
            
            # Show reference files (always)
            ...
            # Show ECO location (verbose only)
            if args.verbose:
        ...
        if not args.quiet:
            ...
            # Show VT statistics (always show if present)
            vt_stats = lib.analyze_vt_statistics(eco_commands)
            if vt_stats:
                vt_str = ", ".join([f"{k}:{v}" for k, v in sorted(vt_stats.items(), 
                                                                   key=lambda x: x[1], reverse=True)])
                print(f"  VT Changes:   {vt_str}")
    ...
    # Print violation examples (always show if violations exist)
    if total_violations['dontuse'] or total_violations['clock']:
        print("\n" + "=" * 79)
        if args.verbose:
        ...
        if total_violations['dontuse']:
            if args.verbose:
                # Show full details (up to 3 examples per cell type)
                print(format_violations_by_type(total_violations['dontuse'], "DontUse Cells"))
            else:
                # Show summary with 1 example per cell type
                print(format_violations_summary(total_violations['dontuse'], "DontUse Cells"))
        ...
        if total_violations['clock']:
            if args.verbose:
                ...
                # Show summary with 1 example per cell type
                print(format_violations_summary(total_violations['clock'], "instNotAllowedOnClocks"))
